...
class CreateContent extends CreateRecord
{
    ...
    public function beforeValidate(): void {
        ...
        $user = Folder::find($this->data['folder_id'])->company->user()->with('mediaSize')->first();

        try {
            if (count($this->data['file_path']) > 0){
                if ($user->plan){
                    if ($user->plan->max_number_of_mb != -1){
                        // $xx =  User::query()->with('mediaSize')->where("id","=",$user->id)->first();
...
//        dd(parent::getFormActions());
        ...
        return array_merge(parent::getFormActions(), [
            ...
            Action::make('createAnother')->action('createAnother')->label('Train and upload another file'),
        ]);
    }
